...
func convertBambooJob(stageName, jobName string, def map[string]any, deps []string, opts Options, result *ConversionResult) Job {
	j := Job{
		ID:           SanitizeJobID(stageName + "_" + jobName),
		Name:         jobName,
	...
	if env := def["environment"]; env != nil {
		if params := bambooEnvParams(env); len(params) > 0 {
			j.Parameters = params
		}
	}
...
func bambooScript(body map[string]any, result *ConversionResult, jobName string) StepResult {
	...
	step := Step{
		...
		WorkingDirectory: stringField(body, "working-dir", ""),
	}
	if interpreter, _ := body["interpreter"].(string); interpreter == "WINDOWS_POWER_SHELL" {
		// TC `type: script` runs cmd.exe on Windows; wrap to dispatch via PowerShell.
		step.ScriptContent = "powershell -Command {\n" + script + "\n}"
	}
...
func bambooEnvParams(v any) map[string]string {
	out := map[string]string{}
	switch env := v.(type) {
	case map[string]any:
		for _, k := range SortedKeys(env) {
			out[k] = MapBambooExpressions(fmt.Sprint(env[k]))
		}
	case string:
		// `environment: 'KEY=val OTHER=val2'` shorthand.
		for kv := range strings.FieldsSeq(env) {
			if i := strings.Index(kv, "="); i > 0 {
				out[kv[:i]] = MapBambooExpressions(kv[i+1:])
			}
		}
	}
	return out
