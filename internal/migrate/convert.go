...
const (
	...
	GitLabCI      SourceCI = "gitlab"
	Jenkins       SourceCI = "jenkins"
	CircleCI      SourceCI = "circleci"
	AzureDevOps   SourceCI = "azure-devops"
	TravisCI      SourceCI = "travis"
	Bitbucket     SourceCI = "bitbucket"
)

var validSources = map[SourceCI]bool{
	GitHubActions: true, GitLabCI: true, Jenkins: true, CircleCI: true,
	AzureDevOps: true, TravisCI: true, Bitbucket: true,
}
...
type Options struct {
	...
	JenkinsURL   string // Jenkins server URL for API-based parsing.
	JenkinsUser  string // Jenkins username for authentication.
	JenkinsToken string // Jenkins API token for authentication.
}
...
func Convert(cfg CIConfig, data []byte, opts Options) (*ConversionResult, error) {
	...
	case GitLabCI:
		result, err = convertGitLab(cfg, data, opts)
	case Jenkins:
		result, err = convertJenkins(cfg, data, opts)
	case CircleCI:
		result, err = convertCircleCI(cfg, data)
	case AzureDevOps:
		result, err = convertAzure(cfg, data, opts)
	case TravisCI:
		result, err = convertTravis(cfg, data)
	case Bitbucket:
		result, err = convertBitbucket(cfg, data)
	default:
