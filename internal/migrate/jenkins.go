...
func convertJenkinsStage(stage stageAST, runner string, prevJobIDs []string, result *ConversionResult) []Job {
	if len(stage.Parallel) > 0 {
		...
		for _, child := range stage.Parallel {
			childJobs := convertJenkinsStage(child, runner, prevJobIDs, result)
	...
	if len(stage.Stages) > 0 {
		...
		for _, child := range stage.Stages {
			childJobs := convertJenkinsStage(child, runner, deps, result)
			jobs = append(jobs, childJobs...)
	...
	if stage.Post != nil {
		postJobs := convertJenkinsPost(stage.Post, runner, []string{j.ID}, result)
...
func convertJenkinsStep(step stepAST, j *Job, result *ConversionResult) {
	...
	case "sh":
		...
		j.Steps = append(j.Steps, Step{
			Name:          "Shell",
			ScriptContent: MapJenkinsVars(script),
		})
	...
	case "echo":
		msg := jenkinsArgValue(args)
		j.Steps = append(j.Steps, Step{
			Name:          "Echo",
			ScriptContent: fmt.Sprintf("echo %q", msg),
		})
