...
func buildAzureJob(id, name string, poolSource map[string]any, steps []any, deps []string, result *ConversionResult, opts Options) Job {
	...
	for _, s := range steps {
		...
		stepEnv := azureStepEnv(step)
		switch {
		case step["script"] != nil:
			script := mapAzureVars(step["script"].(string))
			j.Steps = append(j.Steps, Step{Name: stepName, ScriptContent: script, Parameters: stepEnv})
		case step["bash"] != nil:
...
// azureStepEnv extracts step-level env into a parameter map with Azure
// variable references rewritten to their TeamCity equivalents.
func azureStepEnv(step map[string]any) map[string]string {
	...
	for k, v := range env {
		params[k] = mapAzureVars(fmt.Sprint(v))
	}
	return params
}
