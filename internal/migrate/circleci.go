...
// circleReservedInvocationKeys are the CircleCI workflow job invocation keys
// that do NOT represent user parameter overrides.
var circleReservedInvocationKeys = map[string]bool{
	"name": true, "requires": true, "context": true, "filters": true,
	"type": true, "matrix": true, "pre-steps": true, "post-steps": true,
}
...
func convertCircleCIJobs(doc map[string]any, result *ConversionResult) []Job {
	...
	invocations := collectCircleInvocations(doc, result)
	...
	for _, inv := range invocations {
		...
		// Merge executor-level env (fallback) with job-level env (override).
		params := map[string]string{}
		for k, v := range exec.envVars {
			params[k] = v
		}
		...
		for _, s := range steps {
			...
			case map[string]any:
				convertedSteps, cache := convertCircleStepMap(v, inv.id, commands, inv.params, result)
...
func convertCircleStepMap(v map[string]any, jobID string, commands map[string][]any, invParams map[string]any, result *ConversionResult) ([]Step, bool) {
	hasCache := false
	...
	if run, ok := v["run"].(map[string]any); ok {
		...
		if cmd != "" {
			cmd = mapCircleCIVars(substituteCircleParams(cmd, invParams))
	...
	for key := range v {
		...
		// Merge invocation-level params (fallback) with step-call params (override).
		mergedParams := map[string]any{}
		for k, pv := range invParams {
			mergedParams[k] = pv
		}
		for k, pv := range stepParams {
			mergedParams[k] = pv
		}
		...
		if cmdSteps, ok := commands[key]; ok {
			...
			for _, cs := range cmdSteps {
				if csMap, ok := cs.(map[string]any); ok {
					if run, ok := csMap["run"].(map[string]any); ok {
						if cmd, ok := run["command"].(string); ok {
							cmd = substituteCircleParams(cmd, mergedParams)
							scripts = append(scripts, cmd)
						}
					} else if run, ok := csMap["run"].(string); ok {
						run = substituteCircleParams(run, mergedParams)
						scripts = append(scripts, run)
					}
				}
			}
		...
		if expanded := expandCircleOrbStep(key, mergedParams); expanded != nil {
...
func collectCircleInvocations(doc map[string]any, result *ConversionResult) []circleInvocation {
					...
					if cfg, ok := jobCfg.(map[string]any); ok {
						...
						if requires, ok := cfg["requires"].([]any); ok {
							for _, r := range requires {
								switch v := r.(type) {
								case string:
									inv.deps = append(inv.deps, v)
								case map[string]any:
									if n, ok := v["name"].(string); ok {
										inv.deps = append(inv.deps, n)
									} else if n, ok := v["job"].(string); ok {
										inv.deps = append(inv.deps, n)
									} else {
										result.NeedsReview = append(result.NeedsReview,
											fmt.Sprintf("CircleCI job %q requires entry %v → unrecognized shape, verify dependency manually", inv.id, v))
									}
								}
							}
						}
						...
						for k, v := range cfg {
							if circleReservedInvocationKeys[k] {
								continue
							}
							if inv.params == nil {
