...
func projectScreens(t *testing.T, ts *cmdtest.TestServer) []termbook.Screen {
	return []termbook.Screen{
		...
		termbook.Manual("project-connection-create-github-app", "project connection create github-app", "Wizard: register the App, sign in, install on a repo, then point at vcs create", "teamcity project connection create github-app -p Backend --owner my-org", func(w io.Writer) {
			...
			fmt.Fprintln(w, "  Creates the App on GitHub and brings the credentials back to TeamCity.")
			fmt.Fprintf(w, "  %s On GitHub, click %q.\n", output.Yellow("→"), "Create GitHub App for my-org")
			fmt.Fprintln(w, "Open browser to register the App?  yes  no")
			...
			fmt.Fprintln(w, "  TeamCity calls GitHub as you when testing VCS roots.")
			fmt.Fprintln(w, "Open browser to sign in?  yes  no")
			p.Info("Opening browser to sign in (connection %s)...", "PROJECT_EXT_42")
			...
			fmt.Fprintln(w, "  GitHub Apps are scoped per-repo – pick which ones TeamCity can clone.")
			fmt.Fprintln(w, "Open browser to install the App?  yes  no")
