...
func computeTestDiffs(t1, t2 *api.TestOccurrences) (summaryChanged bool, newFailures []api.TestOccurrence, fixed []string) {
	if t1 == nil || t2 == nil {
		return false, nil, nil
	}

	summaryChanged = t1.Passed != t2.Passed || t1.Failed != t2.Failed || t1.Ignored != t2.Ignored
...
func renderDiff(p *output.Printer, d1, d2 buildData) {
	...
	if renderTestsDiff(p, d1.tests, d2.tests) {
...
func renderTestsDiff(p *output.Printer, t1, t2 *api.TestOccurrences) bool {
	summaryChanged, newFailures, fixed := computeTestDiffs(t1, t2)
	...
	if summaryChanged {
		diffLine(p, "-", "red", "%s", testSummaryStr(t1))
		diffLine(p, "+", "green", "%s", testSummaryStr(t2))
	}
...
func buildDiffJSON(d1, d2 buildData) map[string]any {
	...
	if summaryChanged, newFail, fixed := computeTestDiffs(d1.tests, d2.tests); summaryChanged || len(newFail) > 0 || len(fixed) > 0 {
		tests := map[string]any{
			"from": map[string]int{"passed": d1.tests.Passed, "failed": d1.tests.Failed, "ignored": d1.tests.Ignored},
			"to":   map[string]int{"passed": d2.tests.Passed, "failed": d2.tests.Failed, "ignored": d2.tests.Ignored},
		}
		if len(newFail) > 0 {
