...
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Convert CI configurations to TeamCity pipeline YAML (experimental)",
		Long: `Detect CI/CD configurations in the current repository and convert them
to TeamCity pipeline YAML.

Supported sources: GitHub Actions. Other systems (GitLab, Jenkins, CircleCI,
Azure DevOps, Travis, Bitbucket) will land in follow-up releases.

The generated YAML files can be deployed with:
  teamcity pipeline create <name> --project <id> --file <generated>.tc.yml

Experimental: this is a heuristic converter — always review generated
files before shipping. For higher-quality conversions, run the bundled
...
func runMigrate(ctx context.Context, f *cmdutil.Factory, opts *migrateOptions) error {
	...
	if opts.from != "" {
		...
		if !migrate.ValidSource(filterSource) {
			return fmt.Errorf("unknown CI source %q; supported: github-actions", opts.from)
		}
	}
...
// printConversionStatus reports the honest conversion state: schema failure,
// a clean full conversion, or a partial conversion with counts of unresolved
// items. TC Pipelines YAML can't express matrix strategies, conditional steps,
// service containers, etc., so "partial" is the realistic default.
func printConversionStatus(f *cmdutil.Factory, result *migrate.ConversionResult, dryRun bool) {
	...
	if dryRun {
		return // YAML follows the status line; skip the success indicator to keep output tight.
	}
...
// printExperimentalNotice reminds the user that migration is heuristic,
// points at the higher-quality skill path, and gives them an easy way to
// report regressions with a pre-wired issue template.
func printExperimentalNotice(f *cmdutil.Factory) {
	_, _ = fmt.Fprintf(f.Printer.Out, "\n%s %s — review generated files before shipping.\n",
		output.Yellow("⚠"), output.Bold("Experimental"))
	_, _ = fmt.Fprintf(f.Printer.Out, "  %s run the %s skill with an AI agent.\n",
...
func printConversionResult(f *cmdutil.Factory, cfg migrate.CIConfig, result *migrate.ConversionResult, dryRun bool) {
	...
	// Source/output counts: prefer detection-time numbers for the input side
	// since they reflect the original file faithfully (post-conversion can
	// expand or fold steps).
	srcJobs, srcSteps := cfg.Jobs, cfg.Steps
	if srcJobs == 0 && srcSteps == 0 {
		// Fallback when detection couldn't analyze the file.
